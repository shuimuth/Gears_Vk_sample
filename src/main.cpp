...
class gears_vk_starter_app : public gvk::invokee
{
...
public: // v== avk::invokee overrides which will be invoked by the framework ==v
	gears_vk_starter_app(avk::queue& aQueue)
		: mQueue{ &aQueue }
...
}; // gears_vk_starter_app

int main() // <== Starting point ==
{
	try {
		// Create a window and open it
		auto mainWnd = gvk::context().create_window("Gears-Vk Starter");
		...
		auto app = gears_vk_starter_app(singleQueue);
		// Create another element for drawing the UI with ImGui
		auto ui = gvk::imgui_manager(singleQueue);

		// GO:
		gvk::start(
			gvk::application_name("Gears-Vk Starter App"),
			mainWnd,
			app,
			ui
		);
